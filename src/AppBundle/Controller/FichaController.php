...
class FichaController extends Controller
{
	...
	*/	
	...
	/**
	*@Route("/exportar/interface", name="ficha_setear")
	*@Method({"GET", "POST"})
	*/	
	
	public function exportarIntAction(Request $request)
	{	   
		$defaultData = array('message' => 'Type your message here');
		$form = $this->createFormBuilder()
        ->add('id_ficha_desde', TextType::class)
		->add('id_ficha_hasta', TextType::class)
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			$data = $form->getData();
			$ucd = $data['id_ficha_desde'];
			$uch = $data['id_ficha_hasta'];
       ...
       /*$fichas = $em->getRepository('AppBundle:Ficha')->findAll(); */
	    ...
	    $query = $em->createQuery("SELECT f FROM AppBundle:Ficha f WHERE f.id >= '{$ucd}' and f.id <= '{$uch}'");
	   ...
	   //seteo encabezados
			...
			->setCellValue('AM1', 'Hipertensión Bloqueantes Calcicos')	
			->setCellValue('AN1', 'Hipertensión Furosemida')		
			->setCellValue('AO1', 'Hipertensión Otro')	
			->setCellValue('AP1', 'Hipertensión Cual')			
			->setCellValue('AQ1', 'aas')	
			->setCellValue('AR1', 'Insulina Nph')			
			->setCellValue('AS1', 'Insulina Detemir')	
			->setCellValue('AT1', 'Insulina Corriente')		
			->setCellValue('AU1', 'Insulina Aspártica')	
			->setCellValue('AV1', 'Nº de Inyecciones al Día')		
			->setCellValue('AW1', 'Cobertura Privado')	
			->setCellValue('AX1', 'Cobertura Obra Social')	
			->setCellValue('AY1', 'Cobertura Ninguna')	
			->setCellValue('AZ1', 'Nº Hijos')	
			->setCellValue('BA1', 'Parto Normal')	
			->setCellValue('BB1', 'Parto Prematuro')
			->setCellValue('BC1', 'Parto Cesarea')	
			->setCellValue('BD1', 'cm Hie')	
			->setCellValue('BE1', 'cm Preclampsia')	
			->setCellValue('BF1', 'cm Otras')	
			->setCellValue('BE1', 'cm Cuales');		
	   ...
	   //fin seteo valores
         ...
         return $this->render('ficha/exportar.html.twig', array(
            'form' => $form->createView(),
        ));
